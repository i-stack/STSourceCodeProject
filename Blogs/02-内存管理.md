...
1、iOS下内存管理的基本思想就是引用计数，通过对象的引用计数来对对象的生命周期进行管理，主要有两种方式：
...
1、使用alloc、new、copy、mutableCopy等创建的对象；

2、使用retain保留一个对象；

3、当不在使用这个对象时，需要给这个对象发送一条release或者autorelease消息来释放它。
...
2、当你向一个对象发送retain消息，它的引用计数+1；

3、当你向一个对象发送release消息，它的引用计数-1；

4、当你向一个对象发送autorelease消失，它的引用计数在当前autorelease pool block 结束的时候-1；

5、如果一个对象的引用计数为0，它会被销毁。

### Collections Own the Objects They Contain

1、当向集合类（array、dictionary、set）中添加一个对象时，集合会拥有这个对象；

2、当对象从集合中移除或者集合本身被释放时，会立即给对象发送release消息，对象会被立即释放；

3、如果释放父类对象，那么相关的子类对象也会被释放。
