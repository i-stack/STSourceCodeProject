...
LReturnZero(直接返回)
GetTaggedClass(通过`isa&ISA_MASK`获取Class,GetClassFromIsa)
...
MethodTableLookup(在缓存中没有查找到sel,通过`MethodTableLookup`查找IMP)
lookUpImpOrForward(最终跳转到`lookUpImpOrForward`,此时behavior为LOOKUP_INITIALIZE LOOKUP_RESOLVER)

_objc_msgSend-->cmp-->isSupportTaggedPointer

isSupportTaggedPointer-->isSupport(支持)-->b.le(执行条件判断b.le LNilOrTagged)-->receiveIsNil
isSupportTaggedPointer-->isNotSupport(不支持)-->b.eq(执行条件判断b.eq LReturnZero)-->receiveIsNil

receiveIsNil-->cmpIsNil-->LReturnZero
receiveIsNil-->cmpIsNotNil-->GetTaggedClass-->LGetIsaDone-->CacheLookup(`CacheLookup`缓存查找流程,也就是所谓的sel-imp快速查找流程)-->cache_t-->buckets-->mask(`cache`右移48个字节得到mask)-->tbnzMask(mask第0位是否为0)

tbnzMask-->第0位为1-->LLookupPreopt\Function(跳转到`LLookupPreopt Function`)-->找到IMP就返回,没有找到返回下一个isa,重新执行CacheLookup
tbnzMask-->第0位为0-->bucketIndex-->bucket-->sel

sel-->Y1(是)-->cacheHit
sel-->N1(否)-->selIsNil

selIsNil-->Y2(是)-->MissLabelDynamic,直接返回
