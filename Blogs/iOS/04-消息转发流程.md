...
> > 1、如果reserver为nil:
> >
> > > 如果当前对象是tagged pointer，且reserver为空，则跳转到 LNilOrTagged;
> > > 
> > > > LNilOrTagged 检查reserver是否为nil，如果为nil，直接跳转到LReturnZero并返回；如果reserver不为nil，执行GetTaggedClass后跳转到LGetIsaDone;
> > > > 
> > > > 如果当前对象不是tagged pointer，直接跳转到LReturnZero并返回；
> > > >
> > 2、如果reserver不为nil:
> > 
> > > 执行 CacheLookup 从缓存中查找方法 
