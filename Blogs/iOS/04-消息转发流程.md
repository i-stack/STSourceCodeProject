...
bucketIndex(SEL与上mask=`chche右移48个字节`得到bucket的index)
...
receiveIsNil-->cmpIsNotNil-->GetTaggedClass-->LGetIsaDone-->CacheLookup-->cache_t-->buckets-->mask(`chche右移48个字节`)-->tbnzMask(mask第0位是否为0)

tbnzMask-->第0位为0-->LLookupPreopt\Function(跳转到`LLookupPreopt Function`)
tbnzMask-->第0位为1-->buckets-->bucketIndex-->bucket-->sel
